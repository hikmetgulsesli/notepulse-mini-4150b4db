...
- NotePulse Mini turns the user's request into a directly usable web workflow. The first experience must be the actual web product behavior, not a marketing landing page or placeholder demo.
- - FR-001: Build a tiny single-page utility called NotePulse Mini. It should manage four notes, toggle active/warn/fail states, persist to localStorage, expose window.app state, and export a JSON summary. Keep it static HTML/CSS/JS only.
...
- Four Operations - NotePulse Mini
- Four Editor - NotePulse Mini
- Insights - NotePulse Mini

## SCREEN_SPECS
SCREEN_SPEC_1:
- exact_screen_title: Four Operations - NotePulse Mini
- surface_id: SURF_FOUR_OPERATIONS
- unique_canvas_caption: Four Operations: summary metrics, primary list/board/table, filters, search, selected item preview, empty/loading/error states.
- purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Four data.
...
- data_entities: Four, ActivityEvent, Preference
...
SCREEN_SPEC_2:
- exact_screen_title: Four Editor - NotePulse Mini
- surface_id: SURF_FOUR_EDITOR
- unique_canvas_caption: Four Editor: form fields, required/optional indicators, validation messages, save/cancel controls, unsaved-state feedback.
- purpose: Let the user create, edit, validate, save, cancel, and recover Four changes.
...
- data_entities: Four, ValidationError
- visible_actions: ACT_SAVE_RECORD as form_submit, ACT_CANCEL_EDIT as secondary_button
- entry_exit_rules: SURF_FOUR_OPERATIONS -> Save returns to SURF_FOUR_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
- design_guidance: Form layout must be clear and task-specific; do not invent payment, onboarding, or unrelated identity forms.

SCREEN_SPEC_3:
- exact_screen_title: Insights - NotePulse Mini
...
- purpose: Show useful summaries, trends, and status signals derived from Four data without becoming a separate analytics product.
- required_content: small metrics, recent activity, state distribution, actionable follow-up hints, empty/error state.
- data_entities: Four, ActivityEvent
- visible_actions: ACT_FILTER_INSIGHTS as context_menu, ACT_EXPORT_SUMMARY as secondary_button
- entry_exit_rules: SURF_FOUR_OPERATIONS -> No external BI/admin area; returns to SURF_FOUR_OPERATIONS.
...
## PRODUCT_SURFACES
1. SURF_FOUR_OPERATIONS - Four Operations
   Purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Four data.
   Data: Four, ActivityEvent, Preference
...
2. SURF_FOUR_EDITOR - Four Editor
   Purpose: Let the user create, edit, validate, save, cancel, and recover Four changes.
   Data: Four, ValidationError
   ...
   Entry/exit: SURF_FOUR_OPERATIONS -> Save returns to SURF_FOUR_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
...
3. SURF_INSIGHTS - Insights
   ...
   Entry/exit: SURF_FOUR_OPERATIONS -> No external BI/admin area; returns to SURF_FOUR_OPERATIONS.
